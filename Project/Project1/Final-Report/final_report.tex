...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \REQUIRE tones\_list: list of tone sequences, num\_states: number of possible states (default = 4)
        ...
        \FOR{each $tones$ in $tones\_list$}
            \FOR{$i = 0$ to $length(tones) - 2$}
                \STATE $current\_tone \gets tones[i] - 1$ \COMMENT{Convert tone to zero-based index}
        ...
        \FOR{$j = 0$ to $num\_states - 1$}
            ...
            \IF{$col\_sum > 0$}
                \FOR{$i = 0$ to $num\_states - 1$}
                    \STATE $transition\_counts[i, j] \gets transition\_counts[i, j] / col\_sum$ \COMMENT{Normalize the column}
                \ENDFOR
            \ENDIF
        \ENDFOR
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \REQUIRE $vec1$: a vector, $vec2$: a vector
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \REQUIRE $test\_vector$: a vector, $input\_vec$: a vector
        ...
        \RETURN $norm\_diff$: the infinity norm (maximum absolute difference)
